...
def estimate_next_pos(measurement, OTHER = None):
    ...
    if OTHER == None:
        ...
        OTHER = dict()
        OTHER["angle"] = atan2(measurement[1], measurement[0])
        OTHER["measurement"] = measurement
    else:
        distance_diff = distance_between(measurement, OTHER["measurement"]) # constant after a couple of measurements
        moved_angle = atan2(measurement[1] - OTHER["measurement"][1], measurement[0] - OTHER["measurement"][0])
        angle_diff = moved_angle - OTHER["angle"] # constant after a couple of measurements

        xy_estimate = (measurement[0] + cos(moved_angle + angle_diff) * distance_diff,
            measurement[1] + sin(moved_angle + angle_diff) * distance_diff)

        OTHER["angle"] = moved_angle
        OTHER["measurement"] = measurement
