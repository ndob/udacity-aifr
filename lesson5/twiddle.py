...
def twiddle(tolerance = 0.2): #Make this tolerance bigger if you are timing out!
    params = [0, 0, 0]
    dp = [1, 1, 1]
    count = 0
    best_err = run(params)
    ...
    while sum(dp) > tolerance:
        count += 1
        #print "run", count, best_err
        for i in range(len(params)):
            # run with dp INCREASE
            p_orig = params[i]
            params[i] += dp[i]
            err = run(params)
            if err < best_err:
                ...
                if err < best_err:
                    ...
                    # if error doesn't decrease, reset p to original
                    # and try next round with smaller dp-value
                    params[i] = p_orig
                    dp[i] *= 0.9
