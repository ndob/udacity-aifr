...
def slam(data, N, num_landmarks, motion_noise, measurement_noise):
    ...
    data.insert(0, [[], [world_size / 2.0, world_size / 2.0]])

    for time_index in range(len(data)):
        motion = data[time_index][1]
        measurements = data[time_index][0]

        # measurements (to landmarks)
        for i in range(len(measurements)):
            landmark = measurements[i]
            # index in matrices
            landmark_index = N + landmark[0]
        ...
        if time_index - 1 >= 0:
            xi_x.value[time_index - 1][0] += motion[0] * -1 * motion_confidence
            xi_y.value[time_index - 1][0] += motion[1] * -1 * motion_confidence

        xi_x.value[time_index][0] += motion[0] * motion_confidence
        xi_y.value[time_index][0] += motion[1] * motion_confidence

        omega.value[time_index][time_index] += 1 * motion_confidence
        if time_index - 1 >= 0:
            omega.value[time_index][time_index - 1] += -1 * motion_confidence
            omega.value[time_index - 1][time_index] += -1 * motion_confidence
