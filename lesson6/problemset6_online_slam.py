...
# --------------------------------
#
# online_slam_separated_xi - retains all landmarks but only most recent robot pose
#                            Internally uses x and y xi's to separate matrices.
#

def online_slam_separated_xi(data, N, num_landmarks, motion_noise, measurement_noise):
    dimension = num_landmarks + 1;
    omega = matrix()
    xi_x = matrix()
    xi_y = matrix()
    omega.zero(dimension, dimension)
    xi_x.zero(dimension, 1)
    xi_y.zero(dimension, 1)
    ...
    for data_index in range(len(data)):
        ...
        # add rows and columns for new motion
        keep_indices = range(dimension + 1)
        keep_indices.remove(1)
        omega = omega.expand(dimension + 1, dimension + 1, keep_indices, keep_indices)
        xi_x = xi_x.expand(dimension + 1, 1, keep_indices, [0])
        xi_y = xi_y.expand(dimension + 1, 1, keep_indices, [0])

        motion = data[data_index][1]
        measurements = data[data_index][0]

        # measurements (to landmarks)
        for i in range(len(measurements)):

            landmark = measurements[i]
            # index is hardcoded as we always wish to observe the current position,
            # which is in index 0
            time_index = 0
            # 2 tells that we have currently two positions in omega and xi matrices.
            landmark_index = 2 + landmark[0]

            measurement_confidence = (1.0 / measurement_noise)

            xi_x.value[time_index][0] += landmark[1] * -1 * measurement_confidence
            xi_y.value[time_index][0] += landmark[2] * -1 * measurement_confidence
            xi_x.value[landmark_index][0] += landmark[1] * measurement_confidence
            xi_y.value[landmark_index][0] += landmark[2] * measurement_confidence
            
            omega.value[landmark_index][landmark_index] += 1 * measurement_confidence
            omega.value[time_index][landmark_index] += -1 * measurement_confidence
            omega.value[landmark_index][time_index] += -1 * measurement_confidence
            omega.value[time_index][time_index] += 1 * measurement_confidence

        # motion
        motion_confidence = (1 / motion_noise)

        # x0 (old pos) update
        xi_x.value[0][0] += motion[0] * -1 * motion_confidence
        xi_y.value[0][0] += motion[1] * -1 * motion_confidence

        # x1 (new pos) update
        xi_x.value[1][0] += motion[0] * motion_confidence
        xi_y.value[1][0] += motion[1] * motion_confidence

        omega.value[0][0] += 1 * motion_confidence
        omega.value[0][1] += -1 * motion_confidence
        omega.value[1][0] += -1 * motion_confidence
        omega.value[1][1] += 1 * motion_confidence

        # reduce dimension
        # list contains [1..dimension]
        keep_indices_reduce = range(1, dimension + 1, 1)
        A = omega.take([0], keep_indices_reduce)
        B = omega.take([0], [0])
        Cx = xi_x.take([0], [0])
        Cy = xi_y.take([0], [0])

        omega_prime = omega.take(keep_indices_reduce, keep_indices_reduce)
        xi_x_prime = xi_x.take(keep_indices_reduce, [0])
        xi_y_prime = xi_y.take(keep_indices_reduce, [0])

        omega = omega_prime - (A.transpose() * B.inverse() * A)
        xi_x = xi_x_prime - (A.transpose() * B.inverse() * Cx)
        xi_y = xi_y_prime - (A.transpose() * B.inverse() * Cy)

    omega_inv = omega.inverse()
    mu_x = omega_inv * xi_x
    mu_y = omega_inv * xi_y

    # merge mu_x and mu_y into an interlaced matrix
    mu = matrix()
    mu.zero(dimension * 2, 1)
    cur_idx = 0
    for i in range(len(mu_x.value)):
        mu.value[cur_idx][0] = mu_x.value[i][0]
        cur_idx += 1
        mu.value[cur_idx][0] = mu_y.value[i][0]
        cur_idx += 1
...
def online_slam(data, N, num_landmarks, motion_noise, measurement_noise):
    ...
    for data_index in range(len(data)):
        ...
        # add rows and columns for new motion
        keep_indices = range(dimension + 2)
        keep_indices.remove(2)
        keep_indices.remove(3)
        omega = omega.expand(dimension + 2, dimension + 2, keep_indices, keep_indices)
        xi = xi.expand(dimension + 2, 1, keep_indices, [0])
        motion = data[data_index][1]
        measurements = data[data_index][0]
        ...
        for i in range(len(measurements)):
            ...
            landmark = measurements[i]
            # 2 tells that we have currently two positions in omega and xi matrices.
            landmark_index = (2 + landmark[0]) * 2
            measurement_confidence = (1.0 / measurement_noise)
            
            xi.value[time_index][0] += landmark[1] * -1 * measurement_confidence
            xi.value[time_index + 1][0] += landmark[2] * -1 * measurement_confidence
            xi.value[landmark_index][0] += landmark[1] * measurement_confidence
            xi.value[landmark_index + 1][0] += landmark[2] * measurement_confidence
            ...
            omega.value[landmark_index + 1][landmark_index + 1] += 1 * measurement_confidence
            omega.value[time_index + 1][landmark_index + 1] += -1 * measurement_confidence
            omega.value[landmark_index + 1][time_index + 1] += -1 * measurement_confidence
            omega.value[time_index + 1][time_index + 1] += 1 * measurement_confidence
        ...
        # reduce dimension
        # list contains [2..dimension + 2]
        keep_indices_reduce = range(2, dimension + 2, 1)
        A = omega.take([0, 1], keep_indices_reduce)
        B = omega.take([0, 1], [0, 1])
        C = xi.take([0, 1], [0])

        omega_prime = omega.take(keep_indices_reduce, keep_indices_reduce)
        xi_prime = xi.take(keep_indices_reduce, [0])
...
#result = online_slam_separated_xi(data, N, num_landmarks, motion_noise, measurement_noise)
#print_result(1, num_landmarks, result[0])
...
def solution_check(result, answer_mu, answer_omega, test_omega = True):
...
print "Test online_slam-function"
...
print "Test online_slam_separated_xi-function"
# omega tests fail, but it's not a problem as the end result is correct.
result = online_slam_separated_xi(testdata1, 5, 3, 2.0, 2.0)
solution_check(result, answer_mu1, answer_omega1, False)
result = online_slam_separated_xi(testdata2, 6, 2, 3.0, 4.0)
solution_check(result, answer_mu2, answer_omega2, False)
