...
def doit(initial_pos, move1, move2, Z0, Z1, Z2):
    ...
    for i in range(len(landmarks)):
        ...
        xi.value[i][0] += lm * -1 * confidence
        xi.value[landmark_index][0] += lm * confidence
        xi.show()
        
        omega.value[landmark_index][landmark_index] += 1 * confidence
        omega.value[i][landmark_index] += -1 * confidence
        omega.value[landmark_index][i] += -1 * confidence
        omega.value[i][i] += 1 * confidence
